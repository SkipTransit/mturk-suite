chrome.storage.onChanged.addListener( function (changes) {
  if (changes.settings) DARK_THEME(changes.settings.newValue);
});

chrome.storage.local.get(`settings`, function (result) {
  DARK_THEME(result.settings ? result.settings : null);
});

function DARK_THEME (settings) {
  if (!settings) settings = { theme: false };
  ...
  if (settings.theme) {
    ...
    for (let element of document.querySelectorAll(`.dark`)) document.documentElement.removeChild(element);
  }
}
